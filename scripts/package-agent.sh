...
package_windows() {
    ...
    if [[ ! -f "$binary_path" ]]; then
        log_error "找不�?Windows 二进制文�? $binary_path"
        return 1
    fi
...
Ruinos Agent v${VERSION} - Windows 版本
==================================

安装说明:
1. 以管理员身份运行 install.bat
2. 编辑 config.toml 配置文件
3. 重启服务或手动运�?ruinos-agent.exe

文件说明:
- ruinos-agent.exe: Agent 主程�?- config.toml: 配置文件模板
- install.bat: 安装脚本
- uninstall.bat: 卸载脚本
...
更多信息请访�? https://github.com/your-org/ruinos-agent
EOF
...
package_linux() {
    ...
    local binary_path="$AGENT_DIR/target/$target/release/$binary_name"
    if [[ ! -f "$binary_path" ]]; then
        log_error "找不�?Linux 二进制文�? $binary_path"
        return 1
    fi
...
# Ruinos Agent v${VERSION} - Linux 版本

## 安装说明

1. 解压安装�?2. 运行安装脚本: \`sudo ./install.sh ruinos-agent config.toml --service\`
3. 编辑配置文件: \`sudo nano /etc/ruinos-agent/config.toml\`
4. 重启服务: \`sudo systemctl restart ruinos-agent\`

## 文件说明

- \`ruinos-agent\`: Agent 主程�?- \`config.toml\`: 配置文件模板
- \`install.sh\`: 安装脚本
- \`uninstall.sh\`: 卸载脚本

## 服务管理

- 启动服务: \`sudo systemctl start ruinos-agent\`
- 停止服务: \`sudo systemctl stop ruinos-agent\`
- 查看状�? \`sudo systemctl status ruinos-agent\`
- 查看日志: \`sudo journalctl -u ruinos-agent -f\`
...
package_macos() {
    ...
    local binary_path="$AGENT_DIR/target/$target/release/$binary_name"
    if [[ ! -f "$binary_path" ]]; then
        log_error "找不�?macOS 二进制文�? $binary_path"
        return 1
    fi
    ...
    # 创建 README
    cat > "$package_dir/README.md" << EOF
# Ruinos Agent v${VERSION} - macOS 版本

## 安装说明

1. 解压安装�?2. 运行安装脚本: \`sudo ./install.sh ruinos-agent config.toml --service\`
3. 编辑配置文件: \`sudo nano /etc/ruinos-agent/config.toml\`
4. 重启服务: \`sudo launchctl stop com.example.ruinos-agent && sudo launchctl start com.example.ruinos-agent\`
...
- 启动服务: \`sudo launchctl start com.example.ruinos-agent\`
- 停止服务: \`sudo launchctl stop com.example.ruinos-agent\`
- 查看日志: \`tail -f /var/log/ruinos-agent/ruinos-agent.log\`
...
# 生成校验�?generate_checksums() {
    log_info "生成校验和文�?.."
    
    cd "$DIST_DIR"
    
    # 生成 SHA256 校验�?    if command -v sha256sum >/dev/null 2>&1; then
        sha256sum *.zip *.tar.gz > SHA256SUMS 2>/dev/null || true
    elif command -v shasum >/dev/null 2>&1; then
        shasum -a 256 *.zip *.tar.gz > SHA256SUMS 2>/dev/null || true
    fi
    
    # 生成 MD5 校验�?    if command -v md5sum >/dev/null 2>&1; then
...
# 主函�?main() {
    ...
    echo "Ruinos Agent 打包工具 v${VERSION}"
    echo "=============================="
